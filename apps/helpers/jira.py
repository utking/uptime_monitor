    ...
    def create_ticket(self, project: str, data: dict):
        ...
        try:
            ...
            if existing_issues is None or len(existing_issues) == 0:
                # TODO: checking the same twice or not?!
                if status == triggering_status or status == settings.JIRA_TRIGGERING_STATUS:
                    ...
                    self.__add_comments(post_comments, new_issue.key, issue_dict['output'])
                    if re.match(r'^[\S]+\@[\S]+$', assignee) is not None:
                        self.jira.assign_issue(new_issue, assignee)
            else:
                current_issue = existing_issues[0]
                if status == settings.JIRA_TYPE_OK and current_issue.fields.assignee is None:
